...
// CloudSeed's factory programs, generated from the plugin's
// "Factory Programs/*.json" files. Every preset holds the normalized
// 0..1 values of all parameters in the order of cloudseed::Parameter. The
// plugin stores its parameters as single-precision floats, so every value is
// exactly representable as a float; storing them as floats halves the table's
// size in flash without changing a single bit.
...
struct Preset {
  const char* name;
  float values[kParameterCount];
};

constexpr Preset kChorusDelay = {
    "Chorus Delay",
    {
        0.0,  // InputMix
...
constexpr Preset kNoiseInTheHallway = {
...
constexpr Preset kRubiKaFields = {
    "Rubi-Ka Fields",
    {
...
constexpr Preset kThe90sAreBack = {
    "The 90s Are Back",
    {
        0.0,  // InputMix
        0.0,  // PreDelay
...
constexpr Preset kThroughTheLookingGlass = {
...
// Not a factory program of the plugin: the one built-in program of its
// successor, Ghost Note Audio's Cloud Seed 2, whose MIT-licensed core
// (github.com/GhostNoteAudio/CloudSeedCore, Programs.h) holds it as
// ProgramDarkPlate, converted to this parameter set. The successor scales
// its parameters as the plugin does except for the output and shelf gains
// (decibels there, converted to the plugin's response curve), the low-shelf
// frequency (three octaves there against four) and the seeds (0 to 999,
// converted to the plugin's integers as the successor computes them,
// `floor(value * 999.999)`: tap 333, diffusion 184, delay 218, post
// diffusion 365); its random generator differs, so the
// tap and delay pattern the seeds select is a variant of the successor's,
// not its own. Its multitap and early diffuser are switched off (a "tap
// enabled" switch there, a tap gain of zero and a single tap here), which
// leaves a twelve-line plate: 96 ms lines with four interpolated allpass
// stages each, a 4.8 s decay, a low cut on the input and a high shelf in
// the feedback. The legacy shelf uses its gain as the RBJ A, so its endpoint
// amplitude is gain squared: convert shelf dB with 10^(dB/40), output dB
// with 10^(dB/20), then invert ResponseDec(value, 2). Shelf settings select
// the nearest of the 4001 response entries; the filter's transition shape
// still differs from the successor's EarLevel shelf. Source revision:
// CloudSeedCore deb21ded9eb7dad9b3ff94ce1ba96a963716594e.
constexpr Preset kDarkPlate = {
    "Dark Plate",
    {
        0.23469999432563782,  // InputMix
        0.0,  // PreDelay
        0.6399999856948853,  // HighPass
        0.29330000281333923,  // LowPass
        0.0,  // TapCount
        0.0,  // TapLength
        0.0,  // TapGain
        1.0,  // TapDecay
        0.0,  // DiffusionEnabled
        0.4285714328289032,  // DiffusionStages
        0.3066999912261963,  // DiffusionDelay
        0.7706999778747559,  // DiffusionFeedback
        1.0,  // LineCount
        0.46939998865127563,  // LineDelay
        0.6345999836921692,  // LineDecay
        1.0,  // LateDiffusionEnabled
        0.48799997568130493,  // LateDiffusionStages
        0.23999999463558197,  // LateDiffusionDelay
        0.8506999611854553,  // LateDiffusionFeedback
        0.8904026219451693,  // PostLowShelfGain (-8.88 dB; disabled)
        0.4678437113761902,  // PostLowShelfFrequency
        0.9426395757659525,  // PostHighShelfGain (-4.64 dB)
        0.5133999586105347,  // PostHighShelfFrequency
        0.9759999513626099,  // PostCutoffFrequency
        0.14389999210834503,  // EarlyDiffusionModAmount
        0.2466999888420105,  // EarlyDiffusionModRate
        0.2719999849796295,  // LineModAmount
        0.22929999232292175,  // LineModRate
        0.1467999964952469,  // LateDiffusionModAmount
        0.16669999063014984,  // LateDiffusionModRate
        0.0003330000035930425,  // TapSeed
        0.00018400000443216413,  // DiffusionSeed
        0.00021800000104121864,  // DelaySeed
        0.0003650000144261867,  // PostDiffusionSeed
        0.0,  // CrossSeed
        0.9041702151298523,  // DryOut
        0.0,  // PredelayOut
        0.0,  // EarlyOut
        0.7508186101913452,  // MainOut
        1.0,  // HiPassEnabled
        0.0,  // LowPassEnabled
        0.0,  // LowShelfEnabled
        1.0,  // HighShelfEnabled
